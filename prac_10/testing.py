...
def run_tests():
    ...
    assert test_car.fuel == 10, "Fuel should be 10"
    assert not test_car.fuel == 0, "Fuel should not be default"
...
def format_phrase(phrase):
    ...
    phrase_with_format = phrase.capitalize()
    # add the full stop, but only if we need to
    if phrase_with_format[-1] != '.':
        phrase_with_format += '.'
    return phrase_with_format
